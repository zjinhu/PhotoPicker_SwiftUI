...
struct RangedSliderView<T: View>: View {
    ...
    @ViewBuilder func thumbView(height: CGFloat, position: CGPoint, isLeftThumb: Bool) -> some View {
        ...
        Rectangle()
            ...
            .foregroundColor(.thirdBlack)
            .contentShape(Rectangle())
            .ss.cornerRadius(6, corners: isLeftThumb ? [.topLeft, .bottomLeft] : [.topRight, .bottomRight])
            .overlay(alignment: .center) {
                Image(systemName: isLeftThumb ? "chevron.compact.left" : "chevron.compact.right")
                    .resizable()
                    .frame(width: 6, height: 25, alignment: .center)
            }
